...
(set! updated-list (filter (lambda (file) (isVm file)) file-list))
;(displayln updated-list)

(define folder-name(get-folder-name folder-path))
(define output-asm-file(open-asm-file folder-name))

(when (> (length updated-list) 1)(displayln "more than 1 vm file") 
  (handle-line (string-split(string-trim "call Sys.init 0")) output-asm-file "initialization")
)
...
(for ([file updated-list])
  (when (isVm file) 
      ...
      (displayln (string-append "\n// " file-name " file") output-asm-file)
