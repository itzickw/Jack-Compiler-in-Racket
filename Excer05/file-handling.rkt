...
(define (create-parse-file folder-path file-name input-file-path)
  ...
  (set! output-file-path (string-append folder-path"\\"file-name".vm"))
  (define output-vm-file (open-output-file output-file-path #:exists 'replace))
  (set! parse-string "")

  (read-file input-file-path "" create-parse-string)

  (parsing-line-handling parse-string output-xml-file output-vm-file)
